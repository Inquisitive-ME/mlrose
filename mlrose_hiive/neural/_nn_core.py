...
class _NNCore(_NNBase):
    ...
    def _run_with_ga(self, problem):
        fitness_curve = []
        if self.curve:
            fitted_weights, loss, fitness_curve, call_curve = genetic_alg(
                problem,
    ...
    def _run_with_sa(self, init_weights, num_nodes, problem):
        ...
        if init_weights is None:
            init_weights = np.random.uniform(-1, 1, num_nodes)
        if self.curve:
            fitted_weights, loss, fitness_curve, call_curve = simulated_annealing(
                problem,
    ...
    def __run_with_rhc(self, init_weights, num_nodes, problem):
        ...
        for _ in range(self.restarts + 1):
            ...
            if self.curve:
                current_weights, current_loss, fitness_curve, call_curve = \
